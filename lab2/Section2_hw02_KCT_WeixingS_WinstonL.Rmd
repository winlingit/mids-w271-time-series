...
- Residual plots show that there is no systematic patterns fo residual distribution, except for a few outliners. E(u|x) is slightly away from 0, while the homoskedaticity is highly preserved. Again, there are a few outliers that deviate expected residuals from 0.  The presence of large residuals may indicate overdispersion and imply that an important explanatory variable has been omitted by the model or the distribution used to estimate the model is not appropriate.  More influence of outliners will be discussed in 24.
