...
- Per se laws decrease the fatality rate by about 2%.  The coefficient for perse is not statistically significant (at $\alpha = 0.05$, p-value = 0.18).  Primary seat belt laws appear to have little to no effect on fatality rate as its coefficient is approximately 0.  The coefficient for sbprim is not statistically significant (at $alpha = 0.05$, p-value = 0.998).
...
- The fixed effect estimates are probably more reliable because the model has a higher R-squared (0.73 vs. 0.67).  Also, the estimates for perse and sbprim are larger in magnitude and highly significant, whereas they weren't before, indicating that state unobserved effects may have been correlated with those variables.  One explanation is that policies might have a greater effect on safety when there are unsafe driving conditions.  Thus, states with rainy climates or hilly, windy roads could experience a greater reduction in fatality rate than states with dry climates and flat roads when laws for wearing seat belts and revoking drivers licenses.
...
- A random effects model assumes that the state fixed effects are not correlated with the explanatory variables.  This assumption is probably not valid though, so we would not use the random effects model.  Using the fixed effect model improved the fit of the model and resulted in highly significant estimates, implying some correlation between the fixed effects and explanatory variables.
...
- If the miles driven per capita were to increase by 1000, then the fatality rate would only decrease by 0.5%.

```{r}
exp(.666*log(1000))-100
```
